...
#******
#CHANGE TO THE CORRECT COUNTY NAME HERE
#birthdata_all$data[birthdata_all$location=='Pembina'] <- NA
...
#******
#CHANGE TO THE CORRECT COUNTY NAME HERE
#birthdata_all$data[birthdata_all$location=='Adams'] <- NA
...
## STEP 1: IMPORT & CLEAN DATA
## In order for code chunk to work, must update the file path for the new population vintage estimates AND update the date code matching the most recent year ******* 
```{r}
fips <- fips_codes
...
fullpopdata <- read.csv("/Users/xannaburg/Documents/KidsCountData/Input/demographics/DATADELIVERY_ Vintage 2023 Population Estimates20240701081844/v2023_co_res_char11_nd.csv")
...
#******
#CHANGE TO THE CORRECT COUNTY NAME HERE
#teenbirthdata_all$data[teenbirthdata_all$location=='Divide'] <- NA
...
#******
#CHANGE TO THE CORRECT COUNTY NAME HERE
#birthdata_all$data[birthdata_all$location=='Divide'] <- NA
#birthdata_all$data[birthdata_all$location=='Dunn'] <- NA
#birthdata_all$data[birthdata_all$location=='Bowman'] <- NA
...
#for 2020 and 2021 pop data
population_denominatorsql1 <- paste0("SELECT location, locationtype, timeframe, age_group, data FROM northdakota WHERE vintageyear='",year,"' AND (timeframe='",year,"' OR timeframe='",year_minus1,"') AND varname='childpopulationbysingleyearofage';")

#for 2019 and before
population_denominatorsql2 <- paste0("SELECT location, locationtype, timeframe, age_group, data FROM northdakota WHERE vintageyear='2020' AND (timeframe='",year_minus2,"' OR timeframe='",year_minus3,"' OR timeframe='",year_minus4,"') AND varname='childpopulationbysingleyearofage';")

population_denominator1 <- dbGetQuery(con,population_denominatorsql1)

population_denominator <- dbGetQuery(con,population_denominatorsql2) %>% 
  bind_rows(population_denominator1) %>%
