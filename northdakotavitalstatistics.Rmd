...
#CHANGE TO THE CORRECT COUNTY NAME HERE
#birthdata_all$data[birthdata_all$location=='McKenzie'] <- NA
#birthdata_all$data[birthdata_all$location=='Williams'] <- NA
...
# UPDATE FILE FOR NEW YEAR 
fullpopdata <- read.csv("/Users/xannaburg/Documents/KidsCountData/Input/demographics/DATADELIVERY_ Vintage 2024 Population Estimates20250707010332/v2024_co_res_char11_nd.csv")
...
popdata_county <- fullpopdata %>%
  ...
  subset(RACE11==0) %>%
  subset(HISP==0) %>%
  #subset for recent date ########################UPDATE the date code
  subset(DATE==7) %>%
