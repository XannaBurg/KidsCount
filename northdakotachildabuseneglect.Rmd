...
```{r}
#import the data
ncands <- read_excel(path=paste0("./Input/safety/northdakota_",year,"_ncandschildabuse.xlsx"), guess_max = 1048576,sheet='CPS data') %>%
  
  #rename columns
  rename(assessments='Number of assessments',
         allegedvictims='Number of Alleged Victims',
...
#assessment$data[assessment$location=='Ransom'] <- NA
...
temp_checkregions <- allegedvictims_toupload %>%
  ...
  #******ADD IN THE REGION HERE
  subset(region=='Planning Region 2') %>%
  arrange(data)
view(temp_checkregions)

#******
#CHANGE TO THE CORRECT COUNTY NAME HERE
#allegedvictims_toupload$data[allegedvictims_toupload$location=='Renville'] <- NA
...
#victims$data[victims$location=='Kidder'] <- NA
