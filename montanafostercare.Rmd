...
#input list of counties for MT
MTcounties <- read.csv("./Input/MT County List.csv")
```


## IMPORT THE DATA FILES THAT WILL BE USED
```{r}
fostercaredata <- read_excel(path=paste0("./Input/safety/montana_",year_short,"_fostercaredata.xlsx"),skip=15) 
```
...
fostercaredata_0to17 <- fostercaredata %>% 
  ...
  rename(data=`Youths in Care (0-17 upon entry)`) %>%
  ...
  #update locations
  mutate(location=replace(location,location=='Statewide','Montana')) %>%
  filter(!location %in% c("Blackfeet Tribal Ss*","Crow Tribal Ss","Flathead Tribal Ss (Conf Sk)*","Fort Belknap Tribal Ss As&Gv*",
                          "Northern Cheyenne Tribal Ss*","Rocky Boys Tribal Ss Chip&Cr*")) %>%
  
  #need to add back in missing counties
  mutate(location=replace(location,location=='Sweetgrass','Sweet Grass')) %>%
  full_join(MTcounties,by=c("location"="Location")) %>%
...
fostercaredata_0to5 <- fostercaredata %>% 
  ...
  rename(location=`County`) %>%
  rename(data=`Youths in Care (0-5 upon entry)`) %>%
  select(location,data) %>%
  subset(!is.na(data)) %>%
...
excel0to17_county <- dbGetQuery(con,excel0to17_sql) %>%
  ...
  pivot_wider(names_from='dataformat', values_from=c('2017','2018','2019','2020','2021','2022','2023','2024','2025')) %>%
  select(c(`location`,`2025_Number`,`2025_Rate`,`2024_Number`,`2024_Rate`,`2023_Number`,`2023_Rate`,`2022_Number`,`2022_Rate`,`2021_Number`,`2021_Rate`,
...
```{r}
#edit the data only
Number_sub <- 1581 #substantiated
Number_unsub <- 4730 #unsubstantiated
