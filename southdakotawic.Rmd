...
```{r}
#import the data
wicdata <- read_excel(path=paste0("./Input/economics/southdakota_",year,"_wic.xlsx"),sheet='2021 Monthly by County ') 

wicdata_state <- read_excel(path=paste0("./Input/economics/southdakota_",year,"_wic.xlsx"),sheet='State Avg Month Enrollment') 

###########
#STATE DATA
wic_state <- wicdata_state %>%
  mutate(timeframe=lubridate::year(Month)) %>%
  subset(timeframe==year) %>%
  group_by(timeframe) %>%
  summarise(data=mean(Enrollment)) %>%
...
#COUNTY DATA
wic_county <- wicdata %>%
  full_join(countylist,by=c('Clinic County'='county')) %>%
  rename(location=`Clinic County`) %>%
  group_by(location) %>%
  summarise(data=mean(`Enrollment Count`)) %>%
...
## STEP 1: IMPORT AND CLEAN THE DATA
```{r}
wicracedata <- read_excel(path=paste0("./Input/economics/southdakota_",year,"_wic.xlsx"),sheet='AIAN Enrollment') %>%
  
  mutate(timeframe=lubridate::year(`Month of Reporting Month Date`)) %>%
  mutate(month=lubridate::month(`Month of Reporting Month Date`)) %>%
  subset(timeframe==year) %>%
  group_by(month,timeframe) %>%
  summarise(monthlysum=sum(`Enrollment Count`)) %>%
  ungroup %>%
  group_by(timeframe) %>%
  summarise(data=mean(monthlysum)) %>%
