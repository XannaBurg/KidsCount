...
```{r}
#import the data
wicdata <- read_excel(path=paste0("./Input/economics/southdakota_",year,"_wic.xlsx"),skip=1) 

############
#COUNTY DATA
wic_all <- wicdata %>%
  rename(location=`Household County`,
         data=paste0(year," Average")) %>%
  mutate(locationtype='County') %>%
  mutate(timeframe=year) %>%
  
  mutate(location=replace(location,location=='State WIC Total Enrollment','South Dakota')) %>%
