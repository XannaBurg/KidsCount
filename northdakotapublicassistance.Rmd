...
## Indicator 1: TANF recipients ages 0 through 19
## Indicator 2: SNAP recipients ages 0 through 18
## Indicator 3: Child care assistance recipients ages 0 through 13
## Indicator 4: Medicaid recipients ages 0 through 20
## Indicator 5: Children ages 0 through 18 enrolled in Healthy Steps
...
df_snap <- read_excel(paste0("./Input/economics/northdakota_",fiscalyear,"_economicassistance.xlsx"),sheet="SNAP_1",skip=6)
df_tanf <- read_excel(paste0("./Input/economics/northdakota_",fiscalyear,"_economicassistance.xlsx"),sheet="TANF_2",skip=6)
df_ccap <- read_excel(paste0("./Input/economics/northdakota_",fiscalyear,"_economicassistance_ccap.xlsx")) #this one comes from separate report
df_medicaid <- read_excel(paste0("./Input/economics/northdakota_",fiscalyear,"_economicassistance.xlsx"),sheet="MEDICAID_4",skip=6)
df_healthysteps <- read_excel(paste0("./Input/economics/northdakota_",fiscalyear,"_economicassistance.xlsx"),sheet="Health Steps_CHIP_5",skip=7)
...
df_medicaid2 <- df_medicaid %>%
  rename(location=County,
         data=`Unduplicated Medicaid recipients age 0-20`) %>%
  #remove extra rows or counties not a ND county
  subset(!is.na(location)) %>%
  subset(location != 'County Not Available / Out of State County') %>%
  #remove the word "County"
