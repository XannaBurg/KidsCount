...
df_tanf <- read_excel(path=xl_data, sheet='TANF 0-19', skip=7)
df_snap <- read_excel(path=xl_data, sheet='SNAP 0-18',skip=6)
df_ccap <- read_excel(path=xl_data, sheet='Child Care 0-13',skip=6)
df_medicaid <- read_excel(path=xl_data, sheet='Medicaid 0-20',skip=7)
df_healthysteps <- read_excel(path=xl_data, sheet='Optional Childrens Group 0-19',skip=7)
...
df_snap2 <- df_snap %>%
  ...
  mutate(timeframe=all_of(year)) %>%
  mutate(varname='snapages0to18') %>%
  select(-c(zerotosix,seventoeighteen,`...1`,`...3`))
...
snapages0to18 <- df_snap2 %>%
  ...
  #remove counties that are part of service unit and data matches service location
 # mutate(data=replace(data,location=='Mercer',NA)) %>%
  #mutate(data=replace(data,location=='McLean',NA)) %>%
  #mutate(data=replace(data,location=='Oliver',NA)) %>%
  #mutate(data=replace(data,location=='Sheridan',NA)) 
...
df_tanf2 <- df_tanf %>%
  ...
  select(-c(zerotofive,sixtofifteen,sixteentoseventeen,eighteentonineteen,`...1`,`...3`))
...
df_ccap2 <- df_ccap %>%
  ...
  rename(over13=`Over 13`) %>%
  mutate(locationtype='County') %>%
  mutate(state='North Dakota') %>%
  mutate(data=(undertwo+twoonly+threeonly+fourtofive+sixtothirteen+over13)) %>%
  mutate(dataformat='Number') %>%
  mutate(timeframe=all_of(year)) %>%
  mutate(varname='ccapages0to13') %>%
  select(-c(undertwo,twoonly,threeonly,fourtofive,sixtothirteen,over13,`...1`,`...3`))
...
df_medicaid2 <- df_medicaid %>%
  ...
  select(-c(zerotofive,sixtoeighteen,nineteentotwenty,`...1`,`TOTAL RECIPIENTS`))
...
df_healthysteps2 <- df_healthysteps %>%
  ...
  select(-c(zerotofive,sixtonineteen,`...1`,`...3`))
