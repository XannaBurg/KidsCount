...
```{r}
#import the data
importdata <- read.csv(paste0("./Input/safety/southdakota_",year,"_juvenileadmissionstodrugoralcoholprograms.csv")) 
...
drug_data <- importdata %>%
  ...
  #fix county names
  mutate(location=str_remove_all(County," Total")) %>%
  subset(location!='OUT OF STATE') %>%
  mutate(location=replace(location,location=='Grand','SOUTH DAKOTA')) %>%
