...
```{r}
#import the data
childcaredata <- read_excel(path=paste0("./Input/education/southdakota_",year,"_Child Care County data by category 8.",year,".xlsx"),skip=2) 
...
```{r}
#import the data
childcarecapacity <- read.csv(paste0("./Input/education/southdakota_allyears_childcarecapacity.csv")) 


#STATE DATA
childcare_state <- childcarecapacity %>%
  
  subset(timeframe==year) %>%
...
childcare_statetotals <- childcare_state %>%
  
  group_by(location,locationtype,state,timeframe,varname,dataformat) %>%
...
#COMBINE ALL GEOGRAPHIES
childcare_all <- childcare_state %>%
  bind_rows(childcare_statetotals) %>%
...
## STEP 3: INPUT COUNTY-LEVEL CAPACITY DATA
```{r}
...
childcare_county <- read_excel(path=paste0("./Input/education/southdakota_",year,"_childcarecapacitybycounty.xlsx")) 

childcare_county2 <- childcare_county %>%
  
  #fix formatting so there is a county for each row
  fill(County,.direction=c("down")) %>%
  #remove grand total (we can calculate)
  subset(County != 'Grand Total') %>%
  #remove the word total
  mutate(County=str_remove_all(County," Total")) %>%
  
  #update the category names and remove the total (we can calculate total)
  subset(!is.na(`Provider Type`)) %>%
  mutate(category=case_when(
    `Provider Type`=='Child Care Center' ~ 'Licensed child care centers',
    `Provider Type`=='Family Child Care' ~ 'Registered family child care homes',
    `Provider Type`=='Group Family Child Care' ~ 'Licensed group family child care homes',
    `Provider Type`=='Out of Time School Program' ~ 'Licensed before and after school programs')) %>%
  rename(data=`Total Slots`,
         location=County) %>%
...
childcare_countycapacity <- childcare_county2 %>%
...
#create totals
childcare_countycapacity_total <- childcare_countycapacity %>%
...
childcare_countycapacity2 <- childcare_countycapacity %>%
  bind_rows(childcare_countycapacity_total) %>%
  arrange(location,category)
...
# 1. Print name of location that has a mismatched location ID
if (sum(is.na(childcare_countycapacity2$locationid))>=1) {
  print(childcare_countycapacity2$location[is.na(childcare_countycapacity2$locationid)])
} else if (sum(is.na(childcare_countycapacity2$locationid))==0) {
  'all locations match'
}

# 2. Visually inspect output data
View(childcare_countycapacity2)
```

## STEP 4: ADD TO DATABASE
```{r}
#CHECK DATASET NAMED childcare_countycapacity2 TO VERIFY DATA ISN'T MISSING AND APPEARS CORRECT
#this code adds kids count data to MBPC postgres database
#run postgres code (separate file) first
dbWriteTable(con,database_state,childcare_countycapacity2,append=TRUE,row.names=FALSE)
```

## STEP 5: OUTPUT FILE FOR DATA CENTER
```{r}
#write query from database to get needed format for KC data center
...
## ################################################ ##
## LICENSED BEFORE OR AFTER SCHOOL PROGRAM CAPACITY ##
## ################################################ ##

## STEP 1: IMPORT AND CLEAN DATA
```{r}
#import the data
beforeafterschoolcapacity <- read.csv(paste0("./Input/education/southdakota_allyears_licensedbeforeandafterschoolcapacity.csv")) 


#STATE DATA
beforeafterschool_state <- beforeafterschoolcapacity %>%
...
# 1. Print name of location that has a mismatched location ID
if (sum(is.na(beforeafterschool_state$locationid))>=1) {
  print(beforeafterschool_state$location[is.na(beforeafterschool_state$locationid)])
} else if (sum(is.na(beforeafterschool_state$locationid))==0) {
  'all locations match'
}

# 2. Visually inspect output data
View(beforeafterschool_state)
```

## STEP 2: ADD TO DATABASE
```{r}
#CHECK DATASET NAMED beforeafterschool_state TO VERIFY DATA ISN'T MISSING AND APPEARS CORRECT
#this code adds kids count data to MBPC postgres database
#run postgres code (separate file) first
dbWriteTable(con,database_state,beforeafterschool_state,append=TRUE,row.names=FALSE)
```


## STEP 3: CREATE FILE FOR BEFORE AND AFTER SCHOOL CAPACITY
```{r}
beforeafterschool_countycapacity <- childcare_county2 %>%
  subset(category == 'Licensed before and after school programs') %>%
...
# 1. Print name of location that has a mismatched location ID
if (sum(is.na(beforeafterschool_countycapacity$locationid))>=1) {
  print(beforeafterschool_countycapacity$location[is.na(beforeafterschool_countycapacity$locationid)])
} else if (sum(is.na(beforeafterschool_countycapacity$locationid))==0) {
  'all locations match'
}

# 2. Visually inspect output data
View(beforeafterschool_countycapacity)

```

## STEP 4: ADD TO DATABASE
```{r}
#CHECK DATASET NAMED beforeafterschool_countycapacity TO VERIFY DATA ISN'T MISSING AND APPEARS CORRECT
#this code adds kids count data to MBPC postgres database
#run postgres code (separate file) first
dbWriteTable(con,database_state,beforeafterschool_countycapacity,append=TRUE,row.names=FALSE)
```
