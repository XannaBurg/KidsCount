...
child_data <- read_excel(paste0("./Input/education/northdakota_",year,"_childcaredata_may.xlsx"))

#read in the quality data
quality_data <- read_excel(path=paste0("./Input/education/northdakota_",year,"_childcarequality_may.xlsx"))
...
child_data_2 <- child_data %>%
  ...
  #clean up the data
  #remove any programs that do not have a license number, so we cannot match to county
  subset(!is.na(`File Number`)) %>%
  
  #create new variable matching to KC categories
  #check next year for this next line, in 2023 all the public approvals were school age programs denoted by a K in the license number
  mutate(`Type`=replace(`Type`,`Type`=='Public Approval - License Exempt','School Age License')) %>%
  mutate(category=case_when(
    `Type`=='Center License' | `Type`=='Family License' | 
      `Type`=='Group License-Facility' | `Type`=='Group License-Home' | 
      `Type`=='Multiple License' | 
      `Type`=='Preschool License' ~ 'Licensed',
    `Type`=='School Age License' ~ 'Licensed School Age',
    `Type`=='Self Declared Certificate' ~ 'Self-declaration Providers',
    `Type`=='Public Approval - License Exempt' ~ 'License Exempt')) %>%
  
  mutate(capacity=as.numeric(paste(`Capacity`))) %>%
  
  #merge in county from license number
  mutate(ndcounty_number1=substr(`File Number`,1,2))

child_data_fix <- child_data_2 %>% subset(ndcounty_number1=='0-' | ndcounty_number1=='1-' | ndcounty_number1=='2-' | ndcounty_number1=='3-' | ndcounty_number1=='4-' | ndcounty_number1=='5-') %>%
  mutate(`File Number 2`=str_sub(`File Number`,3)) %>%
  mutate(`File Number 2`=replace(`File Number 2`,`File Number`=='0-0-01898-F','36')) %>%
  mutate(`File Number 2`=replace(`File Number 2`,`File Number`=='1-5-00442-H','05')) %>%
  
  mutate(ndcounty_number2=substr(`File Number 2`,1,2)) %>%
  select(c(`File Number`,ndcounty_number2))


child_data_3 <- child_data_2 %>%
  left_join(child_data_fix,by=c('File Number'='File Number')) %>%
  mutate(ndcounty_number=case_when(
    is.na(ndcounty_number2) ~ ndcounty_number1,
    !is.na(ndcounty_number2) ~ ndcounty_number2)) %>%
  
  mutate(ndcounty_number=as.numeric(paste(ndcounty_number))) %>%
  left_join(countymatch, by=c('ndcounty_number'='ndcounty_number'))
...
```{r}
#COUNTY DATA
countysums_providerscapacity <- child_data_3 %>%
...
countysums_quality <- quality_data %>%
  ...
  select(-c(`...5`,`Row Labels`,`Count of Org Id`)) %>%

  rename(category=`Qris Level Name`,
         location=County,
         capacity=`Capacity`) %>%
  mutate(capacity=as.numeric(paste(capacity))) %>%
...
##Data is not accurate to really do this anymore
```{r}
...
##Data is not accurate enough to do this anymore
```{r}
