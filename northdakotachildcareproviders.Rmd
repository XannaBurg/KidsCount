...
child_data_2 <- child_data %>%
  ...
  #clean up the data
  #match counties using city and zip
  
  #need to fix capitalization of city
  mutate(city_fixed=str_to_title(City)) %>%
  mutate(zip_fixed=as.numeric(Zip)) %>%
  
  mutate(city_fixed=replace(city_fixed,city_fixed=='Lamoure',"LaMoure")) %>%
  mutate(city_fixed=replace(city_fixed,city_fixed=='Mcville',"McVille")) %>%
  mutate(city_fixed=replace(city_fixed,city_fixed=='Mcclusky',"McClusky")) %>%
  
  left_join(countyids,by=c('city_fixed'='City', 'zip_fixed'='Zipcode')) %>%
  ...
  mutate(category=case_when(
    `License Type`=='Center License' | `License Type`=='Family License' | 
      ...
      `License Type`=='Preschool License' | `License Type`=='Provisional Group License' ~ 'Licensed',
