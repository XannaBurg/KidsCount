...
hearingdata <- read_excel(path=paste0("/Users/xannaburg/Documents/KidsCountData/Input/health/northdakota_",year,"_newbornscreeningdata.xlsx"),sheet=paste0(year),skip=6) %>%
  
  #rename columns
  rename(location=`North Dakota Counties`,
         totalbirths=`...14`,
         totalscreened=`Number...15`,
         percentscreened=`Percent of total births...16`,
         totalfailed=`Number...17`,
         percentfailed=`Percent of total screened...18`) %>%
  select(c(location,totalbirths,totalscreened,percentscreened,totalfailed,percentfailed)) %>%
  
  #remove state and state planning regions
  subset(location!='Total For ND Counties:') %>%
  subset(location!='Region 1' & location != 'Region 2' & location !='Region 3' & location !='Region 4' & location!='Region 5' & location !='Region 6' & location !='Region 7' & location !='Region 8') %>%
  subset(!is.na(location)) %>%
  
  mutate(totalscreened=as.numeric(paste(totalscreened)))
```
...
hearing_all <- hearing_county %>%
  ...
  mutate(totalscreened=replace(totalscreened,totalscreened<6,'<6')) %>%
  mutate(percentscreened=replace(percentscreened,totalscreened=='<6','NA')) %>%
  mutate(totalfailed=replace(totalfailed,totalfailed<6,'<6')) %>%
