---
title: "Total population tribal areas"
author: "Xanna Burg"
date: "4/20/2020"
...
acs_tribal3 <- acs_tribal2 %>%
  ...
  select(c(AIANNHNAME,locationtype,DP05_0001E)) %>%
  rename(location=AIANNHNAME,
         data=DP05_0001E) %>%
  mutate(timeframe=year) %>%
  mutate(state=statename) %>%
  mutate(dataformat='Number') %>%
  mutate(varname='totalpopulationtribalarea') %>%
...
```{r}
#query the tribal data with the totalpopulation data
```
