---
title: "Children in the Care of Grandparents "
author: "Xanna Burg"
date: "September 2020"
output: html_document
---

## Indicator 1: Children ages 0 to 17 in the care of grandparents
**Note: this data is not uploaded to MT, ND, SD KIDS COUNT at the county level since the majority of counties have unreliable data. Saving this code in case it is useful in the future**
**For state level estimates: https://datacenter.kidscount.org/data/tables/108-children-in-the-care-of-grandparents?loc=36&loct=2#detailed/2/2-53/false/37,871,870,573,869,36,868,867,133,38/any/433,434 **

**Created by:** Xanna Burg
**Date:** September 2020
**Updated by:**

**Data Source:** U.S. Census Bureau, American Community Survey 5-year Estimates, Table B10002 (numerator) and Table B09001 (denominator)
...
## ################################################# ##
## CHILDREN AGES 0 to 17 IN THE CARE OF GRANDPARENTS ##
## ################################################# ##
...
acs_county <- getCensus(name="acs/acs5",
          ...
          vars=c("B09001_001E","B09001_001M","B10002_002E","B10002_002M"),
          region="county:*",
  ...
  mutate(numerator=(B10002_002E)) %>%
  mutate(numerator_moe=B10002_002M) %>%
  mutate(denominator=B09001_001E) %>%
  mutate(denominator_moe=B09001_001M) %>%
 ...
 #calculate the relative standard error
  ...
  mutate(varname='childreninthecareofgrandparents')
...
acs_tribal <- getCensus(name="acs/acs5",
          vintage=year,
          key=Sys.getenv("CENSUS_API_KEY"),
          vars=c("B10002_002E","B10002_002M","B09001_001E","B09001_001M"),
...
datacenter_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM ", database_state," WHERE timeframe='",acsyear,"' AND varname='childreninthecareofgrandparents';")
...
write.csv(upload_data,file=paste0("../Output/familycommunity/",database_state,"_",year,"_childreninthecareofgrandparents.csv"),row.names=FALSE)


#############################
##OUTPUT MARGIN OF ERROR FILE
moe_sql <- paste0("SELECT location, timeframe, dataformat, data, moe FROM ", database_state," WHERE timeframe='",acsyear,"' AND varname='childreninthecareofgrandparents' AND locationtype='County';")
...
#state
moe_sql2 <- paste0("SELECT location, timeframe, dataformat, data, moe FROM ", database_state," WHERE timeframe='",acsyear,"' AND varname='childreninthecareofgrandparents' AND locationtype='State';")
...
moe_sql3 <- paste0("SELECT location, timeframe, dataformat, data, moe FROM ", database_state," WHERE timeframe='",acsyear,"' AND varname='childreninthecareofgrandparents' AND locationtype='Planning Region';")
...
moe_sql4 <- paste0("SELECT location, timeframe, dataformat, data, moe FROM ", database_state," WHERE timeframe='",acsyear,"' AND varname='childreninthecareofgrandparents' AND locationtype='Tribal Area';")
...
#SAVE OUTPUT DATASET TO UPLOAD TO DATA CENTER
write.csv(moe_output_ordered,file=paste0("../Output/familycommunity/moe/",database_state,"_",year,"_childreninthecareofgrandparents_moe.csv"),row.names=FALSE)
...
datacenter_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM ", database_state," WHERE timeframe='",acsyear,"' AND varname='childreninthecareofgrandparents' AND locationtype <> 'Tribal Area';")
