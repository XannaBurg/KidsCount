...
blsdata_formatted <- blsdata %>%
  ...
  mutate(measure_code=as.numeric(str_sub(`series_id                     `,start=-1))) %>%
...
datacenter_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM ", database_state," WHERE vintageyear='",year,"' AND varname='lausunemployment';")
