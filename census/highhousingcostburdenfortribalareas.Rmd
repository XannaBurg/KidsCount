...
for (i in 1:nrow(tribalcodes1)) {
  ...
  acs_tribal[[i]] <- getCensus(name="acs/acs5/profile",
          ...
          vars=c("DP04_0141PE","DP04_0141PM","DP04_0142PE","DP04_0142PM"),
...
acs_tribal3 <- acs_tribal2 %>%
  ...
  select(c(AIANNHNAME,locationtype,DP04_0141PE,DP04_0141PM,DP04_0142PE,DP04_0142PM)) %>%
  rename(location=AIANNHNAME,
         `30to34_data`=DP04_0141PE,
         `30to34_moe`=DP04_0141PM,
         `35up_data`=DP04_0142PE,
         `35up_moe`=DP04_0142PM) %>%
  
  #combine the two groups to create a 30+ variable
  mutate(`30up_data`=`30to34_data`+`35up_data`,
         `30up_moe`=sqrt((`30to34_moe`^2)+(`35up_moe`^2))) %>%
  select(-c(`30to34_data`,`30to34_moe`,`35up_data`,`35up_moe`)) %>%
  rename(data=`30up_data`,
         moe=`30up_moe`) %>%
  ...
  mutate(varname='highhousingcostburden') %>%
