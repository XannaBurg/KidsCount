...
columnname <- 'Fall ADM Count of Home School Students'
input_dataformat <- 'Number'
...
df_specialed_impairment <- read_excel(path=xl_data, range="A4:X23")
df_specialed_grade <- read_excel(path=xl_data, range="O28:X44")
df_specialed_age <- read_excel(path=xl_data, range="O70:X90")
df_specialed_race <- read_excel(path=xl_data, range="O52:X60")
...
specialed_count <- df_specialed_district %>%
  ...
  select(c('District Name','2021 TOTAL Child Count (ages 3-21)')) %>%
  rename(location='District Name',
         data='2021 TOTAL Child Count (ages 3-21)') %>%
...
specialed_count_state <- df_specialed_district %>%
  rename(data='2021 TOTAL Child Count (ages 3-21)',
...
specialed_count <- df_specialed_district %>%
  subset(!is.na(`District Number`)) %>%
  select(c('District Name','2021 TOTAL Child Count (ages 3-21)','District Number')) %>%
  rename(fulllocation_name='District Name',
         districtid='District Number',
         data='2021 TOTAL Child Count (ages 3-21)') %>%
