...
year <- '24_25'
fullyear <- '2024/25' 
year_prior <- '2023/24' #for dropout data
year_dropout <- '23_24'
calyearformat <- '2025'
...
df_specialed_impairment <- read_excel(path=xl_data, range="A4:AA23")
df_specialed_grade <- read_excel(path=xl_data, range="A25:AA41")
df_specialed_age <- read_excel(path=xl_data, range="A58:AA78")
df_specialed_race <- read_excel(path=xl_data, range="A48:AA56")
...
specialed_count <- df_specialed_district %>%
  ...
  select(c('District Name','2024 TOTAL Child Count (ages 3-21)')) %>%
  rename(location='District Name',
         data='2024 TOTAL Child Count (ages 3-21)') %>%
...
specialed_count_state <- df_specialed_district %>%
  rename(data='2024 TOTAL Child Count (ages 3-21)',
...
specialed_count <- df_specialed_district %>%
  subset(!is.na(`District Number`)) %>%
  select(c('District Name','2024 TOTAL Child Count (ages 3-21)','District Number')) %>%
  rename(fulllocation_name='District Name',
         districtid='District Number',
         data='2024 TOTAL Child Count (ages 3-21)') %>%
