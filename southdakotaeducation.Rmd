...
year <- '20_21'
fullyear <- '2020/21' 
year_prior <- '2019/20' #for dropout data
...
df_specialed_grade <- read_excel(path=xl_data, range="O26:W42")
df_specialed_age <- read_excel(path=xl_data, range="O67:W87")
df_specialed_race <- read_excel(path=xl_data, range="O49:W57")
...
specialed_count <- df_specialed_district %>%
  select(c('District Name','2020 TOTAL Child Count (ages 3-21)')) %>%
  rename(location='District Name',
         data='2020 TOTAL Child Count (ages 3-21)') %>%
...
specialed_count_state <- df_specialed_district %>%
  rename(data='2020 TOTAL Child Count (ages 3-21)',
...
specialed_count <- df_specialed_district %>%
  select(c('District Name','2020 TOTAL Child Count (ages 3-21)','District #')) %>%
  rename(fulllocation_name='District Name',
         districtid='District #',
         data='2020 TOTAL Child Count (ages 3-21)') %>%
...
specialed_percent <- df_specialed_district %>%
  select(c('District Name','Child Count as % of Fall PK-12 Enrollment','District #')) %>%
  rename(fulllocation_name='District Name',
         districtid='District #',
         data='Child Count as % of Fall PK-12 Enrollment') %>%
...
graduation <- c(.4497,.9171,.8421,.6667,.8980,.8288)
completion <- c(.6282,.9330,.8910,.7625,.9429,.8986)
