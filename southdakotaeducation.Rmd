...
year <- '25_26'
fullyear <- '2025/26' 
year_prior <- '2024/25' #for dropout data
year_dropout <- '24_25'
calyearformat <- '2026'
...
enrollment_district <- df_enrollment %>%
  ...
  mutate(location=ifelse(location=='Colome School District 59-3',
                         'Colome Consolidated School District 59-3',location)) %>%
...
columnname <- 'Free & Reduced Lunch Eligibility Percentage'
input_dataformat <- 'Percent'
variablename <- 'freereducedlunch'
...
digestdata_district <- df_digest %>%
  ...
  #add in "School District" so merging works
  mutate(location = str_replace(
    location,
    "(\\d+-\\d+)",
    "School District \\1")) %>%
  ...
  mutate(location=ifelse(location=='Colome School District 59-3',
                         'Colome Consolidated School District 59-3',location)) %>%
  mutate(location=ifelse(location=='Smee School District 15-3',
                         'Wakpala School District 15-3',location)) %>%
...
digest_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM southdakota WHERE varname='",variablename,"' AND locationtype <> 'Tribal Area';")
...
df_specialed_impairment <- read_excel(path=xl_data, range="A4:AB23")
df_specialed_grade <- read_excel(path=xl_data, range="A25:AB41")
df_specialed_age <- read_excel(path=xl_data, range="A58:AB78")
df_specialed_race <- read_excel(path=xl_data, range="A48:AB56")
...
specialed_count <- df_specialed_district %>%
  ...
  select(c('District Name','2025 TOTAL Child Count (ages 3-21)')) %>%
  rename(location='District Name',
         data='2025 TOTAL Child Count (ages 3-21)') %>%
...
specialed_count_state <- df_specialed_district %>%
  rename(data='2025 TOTAL Child Count (ages 3-21)',
...
specialed_districtenrollment_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM southdakota WHERE timeframe='",fullyear,"' AND varname='totalspecialeducation' AND (locationtype='School District' or locationtype='State');")
...
specialed_count <- df_specialed_district %>%
  subset(!is.na(`District Number`)) %>%
  select(c('District Name','2025 TOTAL Child Count (ages 3-21)','District Number')) %>%
  rename(fulllocation_name='District Name',
         districtid='District Number',
         data='2025 TOTAL Child Count (ages 3-21)') %>%
...
upload_data_specialed_impairment <- dbGetQuery(con,specialed_impairment_sql) %>%
  rename(LocationId=locationid,
         ...
         Data=data,
         Category=category) %>%
  subset(Category != "Prolonged Assistance (Ages 0-2)")
...
graduation <- c(.5149,.9427,.8563,.7343,.9237,.8571,.8054)
completion <- c(.6551,.9762,.9045,.7968,.9629,.9126,.8955)
