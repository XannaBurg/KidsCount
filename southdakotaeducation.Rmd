...
year <- '21_22'
fullyear <- '2021/22' 
year_prior <- '2020/21' #for dropout data
...
enrollment_district <- df_enrollment %>%
  ...
  mutate(location=ifelse(location=='Wolsey-Wessington Sch District 02-6',
    ...
    mutate(location=ifelse(location=='Webster Area',
    ...
    mutate(location=ifelse(location=='Faulkton Area',
                         'Faulkton School District 24-4',location)) %>%
    mutate(location=ifelse(location=='Miller',
    ...
    mutate(location=ifelse(location=='Langford Area',
...
xl_data_enroll <- paste0("./Input/education/southdakota_",year,"_enrollmentdropout.xlsx")
df_enrollment <- read_excel(path=xl_data_enroll, sheet='Pubdisgr',skip=3)
df_enrollment_nonpublic <- read_excel(path=xl_data_enroll, sheet='NPdisgr',skip=3)
...
#requested dataset for enrollment and dropouts
xl_data_enroll <- paste0("./Input/education/southdakota_",year,"_enrollmentdropout.xlsx")
df_enrollment <- read_excel(path=xl_data_enroll, sheet='Drop2021SDPubRate',skip=4)
df_enrollment_nonpublic <- read_excel(path=xl_data_enroll, sheet='Drop2021SDNPRate',skip=4)
