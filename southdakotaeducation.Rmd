...
year <- '22_23'
fullyear <- '2022/23' 
year_prior <- '2021/22' #for dropout data
year_dropout <- '21_22'
calyearformat <- '2023'
...
#UPDATE THE RANGES TO INCLUDE THE NEW ROW

#special education state data
xl_data <- paste0("./Input/education/southdakota_",year,"_specialeducationstate.xlsx")
df_specialed_impairment <- read_excel(path=xl_data, range="A4:Y23")
df_specialed_grade <- read_excel(path=xl_data, range="A26:Y42")
df_specialed_age <- read_excel(path=xl_data, range="A67:Y87")
df_specialed_race <- read_excel(path=xl_data, range="A49:Y57")
...
specialed_count <- df_specialed_district %>%
  ...
  select(c('District Name','2022 TOTAL Child Count (ages 3-21)')) %>%
  rename(location='District Name',
         data='2022 TOTAL Child Count (ages 3-21)') %>%
...
specialed_count_state <- df_specialed_district %>%
  rename(data='2022 TOTAL Child Count (ages 3-21)',
...
specialed_count <- df_specialed_district %>%
  subset(!is.na(`District Number`)) %>%
  select(c('District Name','2022 TOTAL Child Count (ages 3-21)','District Number')) %>%
  rename(fulllocation_name='District Name',
         districtid='District Number',
         data='2022 TOTAL Child Count (ages 3-21)') %>%
