...
```{r}
#read in the data
df_enrollment <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='County')

###################
#CREATE COUNTY DATA
county_enrollment <- df_enrollment %>%
  rename(location=`CountyName`) %>%
         mutate(data=`School Enrollment K-12`) %>%
...
```{r}
#read in the data
df_homeschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xlsx"),sheet="Total HomeSchool By CO 2025", range="B3:F60")
df_privateschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xlsx"),sheet="Total Private School By CO 2025", range="B2:F24")
...
specialed <- read_excel(path=paste0("./Input/education/montana_",year,"_specialeducation.xlsx"),sheet="County")

county_key <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='County') %>%
  select(c(CO,CountyName)) %>%
  unique()

#read in the enrollment dataset to calculate pre-K through 12 enrollment
enrollment1 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='County') %>%
  mutate(`School Enrollment K-12`=as.numeric(paste(`School Enrollment K-12`))) %>%
  mutate(`School Enrollment PK`=as.numeric(paste(`School Enrollment PK`))) %>%
  group_by(CountyName) %>%
  summarise(enrollment_prek_12=sum(`School Enrollment K-12`,na.rm=TRUE)+sum(`School Enrollment PK`,na.rm=TRUE))
...
county_specialed <- specialed %>%
  ...
  rename(specialedcount=`Special Education December Count`) %>%
  
  #join enrollment data
  mutate(`CountyName`=replace(`CountyName`,`CountyName`=='Lewis and Clark','Lewis & Clark')) %>%
  left_join(enrollment1,by=c('CountyName'='CountyName')) %>%
  
  #calculate new columns and percent
  rename(enrollmentprek12='enrollment_prek_12') %>%
  mutate(percentspecialed=specialedcount/enrollmentprek12) %>%
  
  #rename to database
  select(c(`CountyName`,specialedcount,percentspecialed)) %>%
  rename(location=`CountyName`,
...
```{r}
#read in the data
frl <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='County')


##In 2024, enrollment and FRL data is in two different datasets, so pull from database for denominator
enrollment_sql <- paste0("SELECT location, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND (varname='publicschoolk12enrollment') AND (timeframe='2024/25');")

k12enrollment <- dbGetQuery(con,enrollment_sql) %>%
  rename(k12enrollment=data)
...
#CREATE COUNTY DATA
county_lunch <- frl %>%
  left_join(k12enrollment,by=c('CountyName'='location')) %>%
  rename(frl='School Enrollment FR',
         location=`CountyName`) %>% 
   mutate(frl=as.numeric(frl)) %>%
   mutate(k12enrollment=as.numeric(k12enrollment)) %>%
   mutate(Number=frl) %>%
...
state_lunch <- frl %>%
 left_join(k12enrollment,by=c('CountyName'='location')) %>%
  mutate(`School Enrollment FR`=as.numeric(`School Enrollment FR`)) %>%
  mutate(k12enrollment=as.numeric(k12enrollment)) %>%
  mutate(location='Montana') %>%
  #calculate percent using K-12 enrollment
  group_by(location) %>%
  summarise(Number=sum(`School Enrollment FR`, na.rm=TRUE),
            Percent=sum(`School Enrollment FR`,na.rm=TRUE)/sum(k12enrollment)) %>%
...
```{r}
#read in the data
dropout <- read_excel(path=paste0("./Input/education/montana_",year,"_education_december.xlsx"),sheet='Dropout Data',skip=11) 
...
county_dropout <- dropout %>%
  ...
  mutate(`Dropout Count`=as.numeric(paste(`Dropout Count`)),
         `Enrollment Count`=as.numeric(paste(`Enrollment Count`))) %>%
  #suppress when dropouts are 5 or fewer
  mutate(`Dropout Count`=replace(`Dropout Count`,`Dropout Count`<=5,NA)) %>%
  mutate(data=`Dropout Count`/`Enrollment Count`) %>%
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
  mutate(state=statename) %>%
...
```{r}
#read in the data
dropout_race <- read_excel(path=paste0("./Input/education/montana_",year,"_education_december.xlsx"),sheet='Dropout Data',range = "A1:D8") 

state_dropout_race <- dropout_race  %>%
  mutate(`Dropout Count`=as.numeric(paste(`Dropout Count`)),
         `Enrollment Count`=as.numeric(paste(`Enrollment Count`))) %>%
  subset(!is.na(`Enrollment Count`)) %>%
  mutate(data=`Dropout Count`/`Enrollment Count`) %>%
  select(c(`Race Ethnicity Category`,data)) %>%
  mutate(timeframe=fullyear) %>%
  mutate(race=case_when(
    `Race Ethnicity Category`=='American Indian' ~ 'American Indian/Alaskan Native',
    `Race Ethnicity Category`=='Asian-American' ~ 'Asian',
    `Race Ethnicity Category`=='African-American' ~ 'Black/African American',
    `Race Ethnicity Category`=='Hispanic' ~ 'Hispanic',
    `Race Ethnicity Category`=='Multi-racial' ~ 'Multiracial',
    `Race Ethnicity Category`=='Native Hawaiian' ~ 
      'Native Hawaiian or Other Pacific Islander',
    `Race Ethnicity Category`=='White, non-Hispanic' ~ 'White')) %>%
  select(-c(`Race Ethnicity Category`)) %>%
  mutate(data=as.numeric(paste(data))) %>%
  mutate(location='Montana') %>%
  mutate(state=statename) %>%
  mutate(dataformat='Percent') %>%
...
##Need to input state estimate in code##
```{r}
grad <- read_excel(path=paste0("./Input/education/montana_",year,"_education_december.xlsx"),sheet='Graduation Data', skip=16) 

###################
#CREATE COUNTY DATA
county_grad <- grad %>%
  rename(location=ExitCountyName) %>%
  mutate(`Cohort Count`=as.numeric(paste(`Cohort Count`)),
         `Graduate Count`=as.numeric(paste(`Graduate Count`))) %>%
  #suppress if 5 or fewer
  mutate(`Graduate Count`=replace(`Graduate Count`,`Graduate Count`<=5,NA)) %>%
  mutate(data=`Graduate Count`/`Cohort Count`) %>%
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
  mutate(state=statename) %>%
...
```{r}
grad_race <- read_excel(path=paste0("./Input/education/montana_",year,"_education_december.xlsx"),sheet='Graduation Data',range="A6:D13") 
...
state_graduates_race <- grad_race %>%
  ...
  mutate(race=case_when(
    ...
    `Race Ethnicity Category`=='Hispanic' ~ 'Hispanic',
    `Race Ethnicity Category`=='Multi-racial' ~ 'Multiracial',
    `Race Ethnicity Category`=='Native Hawaiian or Pacific Islander' ~ 
      'Native Hawaiian or Other Pacific Islander',
    `Race Ethnicity Category`=='White, non-Hispanic' ~ 'White')) %>%
  select(-c(`Race Ethnicity Category`)) %>%
  mutate(data=as.numeric(paste(data))) %>%
...
```{r}
#read in the data
testing1 <- read_excel(path=paste0("./Input/education/montana_",year,"_education_december.xlsx"),sheet='2025 3rd Grade Totals',range="A1:D57") 

###################
#CREATE COUNTY DATA
county_testing1 <- testing1 %>%
  mutate(`Proficient Count`=as.numeric(paste(`Proficient Count`))) %>%
  mutate(`Tested Count`=replace(`Tested Count`,`Tested Count`<=5,NA)) %>%
  mutate(data=`Proficient Count`/`Tested Count`) %>%
  rename(location=co) %>% 
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
...
```{r}
#read in the data
testing3 <- read_excel(path=paste0("./Input/education/montana_",year,"_education_december.xlsx"),sheet='3 yr 3rd Grade Totals',range="A1:E57") 
