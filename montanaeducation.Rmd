...
county_enrollment <- df_enrollment %>%
  rename(location=`County Name`,
         data='County K12 Enrollment Count') %>%
...
```{r}
#read in the data
df_homeschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xls"), sheet='Total HomeSchool By CO 2022')
df_privateschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xls"), sheet='Total Private By CO 2022',skip=1)
...
county_specialed <- specialed %>%
  ...
  left_join(enrollment1,by=c('County Name'='County Name')) %>%
  
  #calculate new columns and percent
  rename(kthrough12='County K12 Enrollment Count',
         prek='County PK EnrollmentCount') %>%
...
state_enrollment <- enrollment2 %>%
  mutate(location='Montana') %>%
  rename(kthrough12='State K12 Enrollment Count',
         prek='State PK EnrollmentCount') %>%
  mutate(prek=as.numeric(prek)) %>%
  mutate(prek=replace(prek,is.na(prek),0)) %>%
  group_by(location) %>%
...
#CREATE COUNTY DATA
county_lunch <- frl %>%
  rename(k12enrollment='County K12 Enrollment Count',
         frl='County  K12 FRL Enrollment Count',
...
```{r}
#read in the data
dropout <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3.xlsx"),sheet='County') 

###################
#CREATE COUNTY DATA
county_dropout <- dropout %>%
  rename(location=ExitCountyName) %>% 
  mutate(`9-12 Dropouts Event`=as.numeric(paste(`9-12 Dropouts Event`)),
         `9-12 Enrolled Event`=as.numeric(paste(`9-12 Enrolled Event`))) %>%
  #suppress when dropouts are 5 or fewer
  mutate(`9-12 Dropouts Event`=replace(`9-12 Dropouts Event`,`9-12 Dropouts Event`<=5,NA)) %>%
  mutate(data=`9-12 Dropouts Event`/`9-12 Enrolled Event`) %>%
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
  mutate(state=statename) %>%
...
```{r}
#read in the data
dropout_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3.xlsx"),sheet='State') 

state_dropout_race <- dropout_race  %>%
  subset(!is.na(`9-12 Enrolled Event`)) %>%
  mutate(`9-12 Dropouts Event`=as.numeric(paste(`9-12 Dropouts Event`)),
         `9-12 Enrolled Event`=as.numeric(paste(`9-12 Enrolled Event`))) %>%
  mutate(data=`9-12 Dropouts Event`/`9-12 Enrolled Event`) %>%
...
##Need to input state estimate in code##
```{r}
grad <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3.xlsx"),sheet='County') 
...
county_grad <- grad %>%
  ...
  mutate(`TotalInCohort`=as.numeric(paste(`TotalInCohort`)),
         `Graduates`=as.numeric(paste(`Graduates`))) %>%
  #suppress if 5 or fewer
  mutate(`Graduates`=replace(`Graduates`,`Graduates`<=5,NA)) %>%
  mutate(data=`Graduates`/`TotalInCohort`) %>%
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
  mutate(state=statename) %>%
...
```{r}
grad_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3.xlsx"),sheet='State') 
...
```{r}
#read in the data
testing1 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata2.xlsx"),sheet='County 3rd Grade Proficiency') 

###################
#CREATE COUNTY DATA
county_testing1 <- testing1 %>%
  rename(data='Percent Proficient and Above ELA',
         location=countyname) %>% 
