...
```{r}
#read in the data
df_enrollment <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='Enrollment')

###################
#CREATE COUNTY DATA
county_enrollment <- df_enrollment %>%
  rename(location=`countyname`) %>%
         mutate(data=`K-12 Enrollment`) %>%
...
```{r}
#read in the data
df_homeschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic2.xlsx"),range="K3:N60")
df_privateschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic2.xlsx"), range="P4:S30")
...
specialed <- read_excel(path=paste0("./Input/education/montana_",year,"_specialeducation.xls"),skip=3)

county_key <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='Enrollment') %>%
  select(c(co,countyname)) %>%
  unique()

#read in the enrollment dataset to calculate pre-K through 12 enrollment
enrollment1 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='Enrollment') %>%
  mutate(`K-12 Enrollment`=as.numeric(paste(`K-12 Enrollment`))) %>%
  mutate(`Pre K Enrollment`=as.numeric(paste(`Pre K Enrollment`))) %>%
  group_by(countyname) %>%
  summarise(enrollment_prek_12=sum(`K-12 Enrollment`,na.rm=TRUE)+sum(`Pre K Enrollment`,na.rm=TRUE))
...
county_specialed <- specialed %>%
  mutate(CO=as.numeric(paste(CO))) %>%
  left_join(county_key,by=c('CO'='co')) %>%
  
    #remove state totals
  subset(!is.na(countyname)) %>%
  subset(`countyname` != '') %>%
  
  #rename special ed count variable
  rename(specialedcount=`Total`) %>%
...
```{r}
#read in the data
frl <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata2.xlsx"),sheet='FRPL by County')


##In 2024, enrollment and FRL data is in two different datasets, so pull from database for denominator
enrollment_sql <- paste0("SELECT location, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND (varname='publicschoolk12enrollment') AND (timeframe='2023/24');")
...
state_lunch <- frl %>%
 left_join(k12enrollment,by=c('CountyName'='location')) %>%
  mutate(`FRPL Enrollment`=as.numeric(`FRPL Enrollment`)) %>%
  mutate(k12enrollment=as.numeric(k12enrollment)) %>%
  mutate(location='Montana') %>%
  #calculate percent using K-12 enrollment
  group_by(location) %>%
  summarise(Number=sum(`FRPL Enrollment`, na.rm=TRUE),
            Percent=sum(`FRPL Enrollment`,na.rm=TRUE)/sum(k12enrollment)) %>%
