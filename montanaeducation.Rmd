...
```{r}
#read in the data
df_homeschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xls"), sheet='Total Home S By CO')
df_privateschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xls"), sheet='Total Private By CO 2021',skip=1)
...
```{r}
#read in the data
dropout <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata_december.xlsx"),sheet='County') 

###################
#CREATE COUNTY DATA
county_dropout <- dropout %>%
  rename(location=CountyName) %>% 
  mutate(`9-12 Dropouts`=as.numeric(paste(`9-12 Dropouts`)),
         `9-12 Enrolled`=as.numeric(paste(`9-12 Enrolled`))) %>%
  #suppress when dropouts are 5 or fewer
  mutate(`9-12 Dropouts`=replace(`9-12 Dropouts`,`9-12 Dropouts`<=5,NA)) %>%
  mutate(data=`9-12 Dropouts`/`9-12 Enrolled`) %>%
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
  mutate(state=statename) %>%
...
```{r}
#read in the data
dropout_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata_december.xlsx"),sheet='State') 

state_dropout_race <- dropout_race  %>%
  rename(data='Event DropoutRate') %>%
...
##Need to input state estimate in code##
```{r}
grad <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata_december.xlsx"),sheet='County') 
...
```{r}
grad_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata_december.xlsx"),sheet='State') 

state_graduates_race <- grad_race %>%
  rename(data='Cohort Graduation Rate') %>%
  select(c(RaceName,data)) %>%
  mutate(timeframe=fullyear) %>%
  mutate(race=case_when(
    RaceName=='American Indian or Alaskan Native' ~ 'American Indian/Alaskan Native',
