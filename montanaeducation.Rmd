...
```{r}
#read in the data
dropout <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3.xlsx"),sheet='DropoutCounty') 
...
```{r}
#read in the data
dropout_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3.xlsx"),sheet='DropoutRace') 
...
##Need to input state estimate in code##
```{r}
grad <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3v2.xlsx"),sheet='GradCounty') 

###################
#CREATE COUNTY DATA
county_grad <- grad %>%
  rename(location=exitcountyname) %>%
  mutate(`TotalInCohort`=as.numeric(paste(`Cohort Count`)),
         `CohortGraduates`=as.numeric(paste(`Cohort Grad`))) %>%
...
```{r}
grad_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3v2.xlsx"),sheet='Grad State - RaceEthnicity') 

state_graduates_race <- grad_race %>%
  mutate(`TotalInCohort`=as.numeric(paste(`Cohort Count`)),
         `Cohort Graduates`=as.numeric(paste(`Cohort Grad`))) %>%
  ...
  select(c(`Student Category`,data)) %>%
  subset(`Student Category` != 'All Students') %>%
  mutate(timeframe=fullyear) %>%
  mutate(race=case_when(
    `Student Category`=='American Indian/Alaska Native' ~ 'American Indian/Alaskan Native',
    `Student Category`=='Asian or Asian-American' ~ 'Asian',
    `Student Category`=='African-American' ~ 'Black/African American',
    `Student Category`=='Hispanic' ~ 'Hispanic',
    `Student Category`=='Multiracial' ~ 'Multiracial',
    `Student Category`=='Native Hawaiian / Pacific Islander' ~ 
      'Native Hawaiian or Other Pacific Islander',
    `Student Category`=='White, non-Hispanic' ~ 'White')) %>%
  select(-c(`Student Category`)) %>%
  mutate(data=as.numeric(paste(data))) %>%
  mutate(location='Montana') %>%
  mutate(state=statename) %>%
  mutate(dataformat='Percent') %>%
...
```{r}
#read in the data
testing1 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3v2.xlsx"),sheet='SBAC County') 

###################
#CREATE COUNTY DATA
county_testing1 <- testing1 %>%
  mutate(ProfELA2023=as.numeric(paste(`Proficient+`))) %>%
  mutate(data=ProfELA2023/TotalTested) %>%
  rename(location=CountyName) %>% 
  select(c(location,data)) %>%
  #add in database variables
  mutate(timeframe=fullyear) %>%
...
```{r}
#read in the data
testing3 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata3v2.xlsx"),sheet='3 year County') 
