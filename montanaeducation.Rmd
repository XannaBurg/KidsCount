...
## Indicator 4: Eligible recipients of free or reduced price lunch by county
## Indicator 5: High school event drop-out rate
## Indicator 6: High school event drop-our rate by race and ethnicity
## Indicator 7: Four-year cohort graduation rate
## Indicator 8: Four-year cohort graduation rate by race and ethnicity
## Indicator 9: Average expendigures per student in public schools
## Indicator 10: Third graders who scored at or above proficient on statewide English Language Arts assessment (1-year totals)
## Indicator 11: Third graders who scored at or above proficient on statewide English Language Arts asssesment (3-year totals)
...
###################
#CREATE REGION DATA
region_enrollment <- county_enrollment %>%
...
```{r}
#read in the data
df_homeschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xlsx"), sheet='Home School',skip=4)
df_privateschool <- read_excel(path=paste0("./Input/education/montana_",year,"_nonpublic.xlsx"), sheet='Private School',skip=3)
...
########################
#REGION HOME SCHOOL DATA
region_homeschool <- df_homeschool %>%
  rename(location=County,
         primaryschool='K-8',
         highschool='9-12') %>%
  subset(!is.na(location)) %>%
  left_join(regionids,by=c('location'='county')) %>%
  group_by(region) %>%
  summarise(data=sum(primaryschool)+sum(highschool)) %>%
  ungroup %>%
  rename(location=region) %>%  
  select(c(location,data)) %>%
  mutate(locationtype='Planning Region') %>%
...
########################
#REGION NON PUBLIC SCHOOL DATA
region_nonpublic <- df_privateschool %>%
  rename(location=County,
         primaryschool='K-8',
         highschool='9-12') %>%
  subset(!is.na(location)) %>%
...
specialed <- read_excel(path=paste0("./Input/education/montana_",year,"_specialeducation.xlsx"))

#read in the enrollment dataset to calculate pre-K through 12 enrollment
enrollment <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='Enrollment Counts')
...
county_specialed <- specialed %>%
  ...
  rename(specialedcount=paste0("SFY20",year," Child Count")) %>%
  
  #join enrollment data
  left_join(enrollment,by=c('County'='CountyName')) %>%
  
  #calculate new columns and percent
  rename(kthrough12='K-12 Enrollment Count',
         prek='PreK Enrollment Count') %>%
  mutate(prek=as.numeric(prek)) %>%
  mutate(prek=replace(prek,is.na(prek),0)) %>%
  mutate(enrollmentprek12=kthrough12+prek) %>%
  
  mutate(percentspecialed=specialedcount/enrollmentprek12) %>%
  
  #rename to database
  select(c(County,specialedcount,percentspecialed)) %>%
  rename(location=County,
...
#create summary of statewide enrollment
state_enrollment <- enrollment %>%
  ...
  mutate(prek=replace(prek,is.na(prek),0)) %>%
  group_by(location) %>%
  summarise(enrollmentprek12=sum(kthrough12)+sum(prek)) %>%
  ungroup
  
  
state_specialed <- specialed %>%
  
  #keep only state totals
  subset(County=='State') %>%
  ...
  #join enrollment data
  left_join(state_enrollment,by=c('location'='location')) %>%
  
  #calculate new columns and percent
  mutate(percentspecialed=specialedcount/enrollmentprek12) %>%
  
  #rename to database
  select(c(location,specialedcount,percentspecialed)) %>%
  rename(Percent=percentspecialed,
         Number=specialedcount) %>%
  mutate(state=statename) %>%
  mutate(locationtype='State') %>%
  mutate(timeframe=fullyear) %>%
...
##########################
#CREATE THE REGION DATASET
region_specialed <- specialed %>%
  ...
  group_by(region) %>%
  summarise(Number=sum(specialedcount),
            Percent=sum(specialedcount)/sum(enrollmentprek12)) %>%
  ungroup %>%
  

  #rename to database
  select(c(region,Number,Percent)) %>%
  rename(location=region) %>%
...
```{r}
#read in the data
frl <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='Enrollment Counts')

###################
#CREATE COUNTY DATA
county_lunch <- frl %>%
  rename(k12enrollment='K-12 Enrollment Count',
         frl='K-12 FRL Enrollment Count',
         location=CountyName) %>% 
  mutate(Number=frl) %>%
...
state_lunch <- frl %>%
  mutate(location='Montana') %>%
   rename(k12enrollment='K-12 Enrollment Count',
         frl='K-12 FRL Enrollment Count') %>% 
  group_by(location) %>%
  summarise(Number=sum(frl),
            Percent=sum(frl)/sum(k12enrollment)) %>%
  #add in database variables
  mutate(state=statename) %>%
  mutate(locationtype='State') %>%
  mutate(timeframe=fullyear) %>%
...
```{r}
#read in the data
dropout <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='County Dropout') 
...
```{r}
#read in the data
dropout_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='State Dropout Race-Ethnicity Co') 
...
state_dropout_race <- dropout_race  %>%
  ...
  mutate(race=case_when(
    Name=='American Indian or Alaskan Native' ~ 'American Indian/Alaskan Native',
    Name=='Asian' ~ 'Asian',
    Name=='Black or African American' ~ 'Black/African American',
    Name=='Hispanic' ~ 'Hispanic',
    Name=='Multi-Racial' ~ 'Multiracial',
...
##Need to input state estimate in code##
```{r}
grad <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='County Graduation') 
...
```{r}
grad_race <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='Race Graduates') 

state_graduates_race <- grad_race %>%
  rename(data='Grad Rate') %>%
  select(c(Name,data)) %>%
  mutate(timeframe=fullyear) %>%
  mutate(race=case_when(
    Name=='American Indian or Alaskan Native' ~ 'American Indian/Alaskan Native',
...
```{r}
#read in the data
testing1 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='SBAC 2015') 

###################
#CREATE COUNTY DATA
county_testing1 <- testing1 %>%
  rename(data='Proficient and Above Percent',
         location=CountyName) %>% 
  select(c(location,data)) %>%
  #remove the asterisks and assign NA
  mutate(data=replace(data,data=='*',NA)) %>%
