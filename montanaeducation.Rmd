...
```{r}
#read in the data
df_enrollment <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='response')

###################
#CREATE COUNTY DATA
county_enrollment <- df_enrollment %>%
  rename(location=`CountyName`) %>%
         mutate(data=`Enrollment K-12`) %>%
  select(c(location,data)) %>%
  subset(location!="COMBINED STATE") %>%
  group_by(location) %>%
...
specialed <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='response') %>%
  subset(CountyName!="COMBINED STATE") 


#read in the enrollment dataset to calculate pre-K through 12 enrollment
enrollment1 <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='response') %>%
  mutate(`Enrollment K-12`=as.numeric(paste(`Enrollment K-12`))) %>%
  mutate(`Enrollment PK`=as.numeric(paste(`Enrollment PK`))) %>%
  subset(CountyName!="COMBINED STATE") %>%
  group_by(CountyName) %>%
  summarise(enrollment_prek_12=sum(`Enrollment K-12`,na.rm=TRUE)+sum(`Enrollment PK`,na.rm=TRUE))
...
```{r}
#read in the data
frl <- read_excel(path=paste0("./Input/education/montana_",year,"_educationdata.xlsx"),sheet='response')
...
county_lunch <- frl %>%
  ...
  subset(location!="COMBINED STATE") %>%
   ...
   mutate(k12enrollment=as.numeric(`Enrollment K-12`)) %>%
   mutate(Number=frl) %>%
...
state_lunch <- frl %>%
  ...
  #calculate percent using K-12 enrollment
  group_by(location) %>%
  summarise(Number=sum(`frl`, na.rm=TRUE),
            Percent=sum(`frl`,na.rm=TRUE)/sum(k12enrollment)) %>%
...
lunch_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM montana WHERE timeframe='",fullyear,"' AND varname='freereducedlunch' AND dataformat='Percent';")
