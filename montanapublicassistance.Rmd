...
## Indicator 4: SNAP average benefit amount per household
## Indicator 5: TANF recipients and TANF households
## Indicator 6: TANF recipients ages 0 to 18
...
medicaid <- read_excel(path=paste0("./Input/health/montana_June ",year," Enrollment_MednCHIP County Table.xlsx"),sheet="To Date All Enrollment",skip=4) %>%
...
medicaid_region_number <- medicaid_county %>%
  
  #add in regions
  left_join(regionids,by=c('COUNTY'='county')) %>%
  
  group_by(region) %>%
  summarise(chip=sum(chip),
            medicaid_group=sum(medicaidchildren)+sum(medicaidandchip),
            totalchildren=sum(medicaid_group)+sum(chip)) %>%

  ungroup %>%
  
  select(c(region,medicaid_group,chip,totalchildren)) %>%
...
###############################################################
###############################################################
#UNION COUNTY, STATE, AND STATE PLANNING REGION for number only
upload_medicaid_number <- medicaid_county_number %>%
  bind_rows(medicaid_state_number) %>%
  bind_rows(medicaid_region_number) %>%
...
```{r}
#read in the data that was shared by DPHHS
snap <- read_excel(path=paste0("./Input/economics/montana_june_17_19_tanfsnap.xlsx"),sheet=paste0("SNAP June ",year))
...
#####################
#CLEAN UP REGION DATA
snap_region_rechouse <- snap_county_rechouse %>%
...
upload_snap_rechouse <- snap_county_rechouse %>%
  ...
  bind_rows(snap_region_rechouse) %>%
...
#####################
#CLEAN UP REGION DATA
snap_region_number <- snap_county_number %>%
...
```{r}
#read in the data that was shared by DPHHS
tanf <- read_excel(path=paste0("./Input/economics/montana_june_17_19_tanfsnap.xlsx"),sheet=paste0("TANF June ",year))
...
#####################
#CLEAN UP REGION DATA
tanf_region_rechouse <- tanf_county_rechouse %>%
...
upload_tanf_rechouse <- tanf_county_rechouse %>%
  ...
  bind_rows(tanf_region_rechouse) %>%
...
#####################
#CLEAN UP REGION DATA
tanf_region_number <- tanf_county_number %>%
