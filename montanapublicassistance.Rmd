...
medicaid <- read_excel(path=paste0("./Input/economics/montana_",year,"_tanfsnapmedicaid.xlsx"),sheet=paste0("Medicaid June ",year)) %>%
  rename(medicaidchildren='Medicaid Children',
         chip='HMK Children',
         medicaidandchip='Medicaid & CHIP Children')

#create state data frame subsetting for Grand Total row
medicaid_state <- medicaid %>%
  subset(County=='State Total')

#create the county data frame subsetting to remove NA counties and cities
medicaid_county <- medicaid %>%
  #remove county number from string
  mutate(location=substr(County, 1, nchar(County) - 5)) %>% 
  mutate(location=replace(location, 
                          statename=='Montana' & location=='Lewis And Clark', 
                          'Lewis & Clark')) %>%
  mutate(location=replace(location, 
                          statename=='Montana' & location=='Mccone', 
                          'McCone')) %>%
...
medicaid_county_number <- medicaid_county %>%
  group_by(location) %>%
  summarise(medicaid_group=sum(c(medicaidchildren,medicaidandchip),na.rm=TRUE),
            chip=sum(chip,na.rm=TRUE),
            totalchildren=sum(c(medicaid_group,chip),na.rm=TRUE)) %>%
  
  select(c(location,medicaid_group,chip,totalchildren)) %>%
