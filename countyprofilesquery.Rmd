...
#####CHILDREN NEEDING CARE SERVED BY LICENSED CAPACITY
countydata8_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childcarecapacitybytype' AND dataformat='Percent' AND (timeframe='",year_current,"' OR timeframe='",year_minus1,"');")
...
#UPDATE THE YEAR TO MATCH THE CURRENT YEAR
year_current <- '2020'
#manually input the statewide FRL, not in the database yet
frl_statevalue <- '0.xxx'
...
countydata9_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='licensedorregisteredchildcare' AND category='Total' AND dataformat='Number' AND (timeframe='",year_current,"' OR timeframe='",year_minus1,"');")

countydata9 <- dbGetQuery(con,countydata9_sql) %>%
  mutate(varname='Licensed Child Care Facilities') %>%
  mutate(age_group='Ages 0 to 13') %>%
  mutate(date_description='State Fiscal Year') %>%
...
#####LICENSED OR REGISTERED CHILD CARE
countydata10_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='licensedbeforeandafterschoolprograms' AND dataformat='Number' AND (timeframe='",year_current,"' OR timeframe='",year_minus1,"');")

countydata10 <- dbGetQuery(con,countydata10_sql) %>%
  mutate(varname='Licensed Before or After School Programs') %>%
...
#current year for SD will not have a value, give it a NA
countydata11_second <- data.frame(frl_statevalue) %>%
  rename(data=frl_statevalue) %>%
  mutate(location='South Dakota',
         timeframe=schoolyear_current,
         varname='Free or Reduced-Price Lunch Participation',
         age_group='Pre-K through 12 grades',
         date_description='School Year',
         data_source='South Dakota Department of Education, Child and Adult Nutrition Services',
         data_notes='Data is not available if value approached zero or 100, or if 20 or fewer children are enrolled in the county.',
         date_mostrecent=1,
         dataformat='Percent_range') %>%
