...
threeyear_minus4 <- as.character(paste0(year_minus6,"-",year_minus4))
...
countydata9_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_current,"' OR timeframe='",schoolyear_minus3,"');")
...
countydata9 <- dbGetQuery(con,countydata9_sql) %>%
  ...
  mutate(date_mostrecent=case_when(
    ...
    timeframe==all_of(schoolyear_minus3) ~0)) %>%
  mutate(dataformat='Percent') 


#####FOUR-YEAR COHORT GRADUATION RATE
countydata10_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND varname='graduationrate4cohort' AND dataformat='Percent' AND (timeframe='",schoolyear_current,"' OR timeframe='",schoolyear_minus1,"');")
...
race <- c("Overall","American Indian/Alaska Native","Asian","Black","Latinx","2+ Races","Native Hawaiian or Other Pacific Islander","White")
data <- c(.856,.66,.961,.785,.794,.79,.818,.893)
countydata10_byrace <- data.frame(race,data) %>%
  mutate(location='Montana',
         ...
         data_notes='Data is not reported if the number of students enrolled in the cohort is less than 6.',
         date_mostrecent=1,
         dataformat='Percent') %>%
  mutate(data=as.character(paste(data)))
#####---------------------------------#####
...
countydata11_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND (varname='thirdgradestateelaproficiency1year' OR varname='thirdgradestateelaproficiency3year') AND dataformat='Percent' AND (timeframe='",schoolyear_current,"' OR timeframe='",schoolyear_minus1,"');")

reading_currentyear <- dbGetQuery(con,countydata11_sql) %>%
  subset(timeframe==schoolyear_current) %>%
  pivot_wider(id_cols=c('location'),names_from=timeframe,values_from=data) %>%
  rename(data=all_of(schoolyear_current)) %>%
  mutate(timeframe=schoolyear_current) %>%
  mutate(date_mostrecent=1)

reading_prioryear <- dbGetQuery(con,countydata11_sql) %>%
  subset(timeframe==schoolyear_minus1) %>%
  pivot_wider(id_cols=c('location'),names_from=timeframe,values_from=data) %>%
  rename(data=all_of(schoolyear_minus1)) %>%
  mutate(timeframe=schoolyear_minus1) %>%
  mutate(date_mostrecent=0)


#CODE THAT WILL PULL 3 YEAR DATA TOO - can start using again in 2023
#reading_currentyear <- dbGetQuery(con,countydata11_sql) %>%
 # subset(timeframe==schoolyear_current | timeframe==threeyear_minus3) %>%
  #pivot_wider(id_cols=c('location'),names_from=timeframe,values_from=data) %>%
  ...
  #mutate(timeframe=case_when(
   # !is.na(oneyear) ~ schoolyear_minus1,
    #is.na(oneyear) ~ threeyear_minus3)) %>%
  #select(c(location,timeframe,data)) %>%
  #mutate(date_mostrecent=1)

#reading_prioryear <- dbGetQuery(con,countydata11_sql) %>%
  #subset(timeframe==schoolyear_minus3 | timeframe==threeyear_minus4) %>%
  #pivot_wider(id_cols=c('location'),names_from=timeframe,values_from=data) %>%
  
  #create new variable using mixture of 1 and 3 year
  #rename(oneyear=all_of(schoolyear_minus3),
   #      threeyear=all_of(threeyear_minus4)) %>%
  #mutate(data=case_when(
  #  !is.na(oneyear) ~ oneyear,
  #  is.na(oneyear) ~ threeyear)) %>%
  #mutate(timeframe=case_when(
  #  !is.na(oneyear) ~ schoolyear_minus3,
  #  is.na(oneyear) ~ threeyear_minus4)) %>%
  #select(c(location,timeframe,data)) %>%
  #mutate(date_mostrecent=0)
...
countydata12_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND varname='childrenwithparentsinlaborforce' AND dataformat='Percent' AND age_group='Ages 0-17' AND (timeframe='",fiveyear_minus1,"' OR timeframe='",fiveyear_minus2,"');")
...
countydata13_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND varname='saipe_childreninpoverty' AND dataformat='Percent' AND age_group='Ages 0-17' AND (timeframe='",year_minus1,"' OR timeframe='",year_minus2,"');")
...
countydata14 <- dbGetQuery(con,countydata14_sql) %>%
  ...
  mutate(age_group='Ages 0 to 17') %>%
  mutate(date_description='Calendar Year') %>%
  mutate(data_source='Feeding America, Map the Meal Gap') %>%
  mutate(data_notes='') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_minus2) ~ 1,
    timeframe==all_of(year_minus3) ~0)) %>%
...
threeyear_minus3 <- as.character(paste0(year_minus5,"-",year_minus3))
threeyear_minus4 <- as.character(paste0(year_minus6,"-",year_minus4))
threeyear_minus5 <- as.character(paste0(year_minus7,"-",year_minus5))
twoyear_currentyear <- as.character(paste0(year_minus1,"-",year_current))
twoyear_minus1 <- as.character(paste0(year_minus2,"-",year_minus1))
fiveyear_minus1 <- as.character(paste0(year_minus5,"-",year_minus1))
fiveyear_minus2 <- as.character(paste0(year_minus6,"-",year_minus2))
...
#####WOMEN WHO RECEIVE EARLY PRENATAL CARE
race <- c("Overall","American Indian/Alaska Native","Asian","Black","White","2+ Races or Other")
data <- c(.777,.403,.709,.598,.837,.677)
countydata5_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2018-2022',
         varname='Women Who Receive Early Prenatal Care by Race',
...
#####CHILD CARE CAPACITY
countydata7_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childcarecapacitybytype' AND dataformat='Number' AND (timeframe='",year_current,"' OR timeframe='",year_minus1,"');")
...
countydata7 <- dbGetQuery(con,countydata7_sql) %>%
  ...
  mutate(date_description='2022 data is a snapshot from February. 2023 data is a snapshot from May') %>%
    mutate(data_source='North Dakota Department of Human Services') %>%
  mutate(data_notes='Capacity data for 2023 and 2022 includes only licensed facilities or licensed school age programs.') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_current) ~ 1,
    timeframe==all_of(year_minus1) ~0)) %>%
  mutate(dataformat='Number')
...
countydata8_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childcareprovidersbytype' AND dataformat='Number' AND (timeframe='",year_current,"' OR timeframe='",year_minus1,"');")
...
countydata8 <- dbGetQuery(con,countydata8_sql) %>%
  ...
  ungroup %>%
  mutate(data=as.character(paste(data))) %>%
  mutate(varname='Licensed Child Care Providers') %>%
  mutate(age_group='Ages 0 to 13') %>%
  mutate(date_description='2022 data is a snapshot from February. 2023 data is a snapshot from May') %>%
    mutate(data_source='North Dakota Department of Human Services') %>%
  mutate(data_notes='In 2022, providers counted include licensed facilities, licensed school age programs, registered tribal programs, self-declaration providers, and approved relatives. In 2023, providers include licensed facilities, licensed school age programs, and self-declaration providers.') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_current) ~ 1,
    timeframe==all_of(year_minus1) ~0)) %>%
  mutate(dataformat='Number')
...
#####FREE OR REDUCED-PRICE LUNCH PARTICIPATION
countydata9_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_current,"');")
...
data <- c(.843,.649,.706,.724,.891,.864,.717)
countydata10_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2021/22',
         varname='Four-Year Cohort Graduation Rate by Race',
...
#####3RD GRADE STUDENTS WHO SCORED PROFICIENT IN READING
countydata11_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND (varname='thirdgradestatereadingproficiency1year' OR varname='thirdgradestatereadingproficiency3year') AND dataformat='Percent' AND (timeframe='",schoolyear_minus1,"' OR timeframe='",schoolyear_minus2,"');")


countydata11 <- dbGetQuery(con,countydata11_sql) %>%
  mutate(varname='3rd Grade Students Who Scored Proficient in Reading') %>%
  mutate(age_group='3rd graders') %>%
  mutate(date_description='School Year') %>%
  mutate(data_source='North Dakota Department of Public Instruction') %>%
  mutate(data_notes='Data is not reported if the number of students taking the assessment is less than 10 or if the percentage apprached zero or 100.') %>%
  mutate(dataformat='Percent_range') %>%
  #for range values, create the character variable that includes the percentage
  mutate(data_truncated=as.numeric(paste(data))) %>%
  mutate(data_truncated=data_truncated*100) %>%
...
#####CHILDREN WITH ALL PARENTS WORKING
countydata12_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childrenwithparentsinlaborforce' AND dataformat='Percent' AND age_group='Ages 0-17' AND (timeframe='",fiveyear_minus1,"' OR timeframe='",fiveyear_minus2,"');")
...
#####CHILDREN LIVING IN POVERTY
countydata13_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='saipe_childreninpoverty' AND dataformat='Percent' AND age_group='Ages 0-17' AND (timeframe='",year_minus1,"' OR timeframe='",year_minus2,"');")
...
data <- c(.113,.384,.065,.174,.333,.244)
countydata13_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2018-2022',
         varname='Children Living in Poverty by Race',
...
#####FREE OR REDUCED-PRICE LUNCH PARTICIPATION
countydata11_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_current,"');")
...
#####CHILDREN WITH ALL PARENTS WORKING
countydata12_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childrenwithparentsinlaborforce' AND dataformat='Percent' AND age_group='Ages 0-17' AND (timeframe='",fiveyear_minus1,"' OR timeframe='",fiveyear_minus2,"');")
...
#####CHILDREN LIVING IN POVERTY
countydata13_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='saipe_childreninpoverty' AND dataformat='Percent' AND age_group='Ages 0-17' AND (timeframe='",year_minus1,"' OR timeframe='",year_minus2,"');")
