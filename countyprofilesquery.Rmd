...
threeyear_current <- as.character(paste0(year_minus2,"-",year_current))
...
data <- c(.795,.501,.797,.734,.829,.756)
...
countydata9_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_current,"' OR timeframe='",schoolyear_minus1,"');")
...
data <- c(.852,.687,.907,.812,.783,.824,.826,.881)
...
countydata11_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND (varname='thirdgradestateelaproficiency1year' OR varname='thirdgradestateelaproficiency3year') AND dataformat='Percent' AND (timeframe='",schoolyear_current,"' OR timeframe='",schoolyear_minus1,"' OR timeframe='",threeyear_current,"' OR timeframe='",threeyear_minus1,"');")

#CODE THAT WILL PULL 3 YEAR DATA
reading_currentyear <- dbGetQuery(con,countydata11_sql) %>%
  subset(timeframe==schoolyear_current | timeframe==threeyear_current) %>%
  pivot_wider(id_cols=c('location'),names_from=timeframe,values_from=data) %>%
  
  #create new variable using mixture of 1 and 3 year
  rename(oneyear=all_of(schoolyear_current),
         threeyear=all_of(threeyear_current)) %>%
  mutate(data=case_when(
    !is.na(oneyear) ~ oneyear,
    is.na(oneyear) ~ threeyear)) %>%
  mutate(timeframe=case_when(
    !is.na(oneyear) ~ schoolyear_current,
    is.na(oneyear) ~ threeyear_current)) %>%
  select(c(location,timeframe,data)) %>%
  mutate(date_mostrecent=1)
...
reading_prioryear <- dbGetQuery(con,countydata11_sql) %>%
  ...
  mutate(timeframe=case_when(
    ...
    is.na(oneyear) ~ threeyear_minus1)) %>%
  select(c(location,timeframe,data)) %>%
  mutate(date_mostrecent=0)
...
countydata4_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND (varname='medicaidages0to20' OR varname='healthystepsages0to18') AND dataformat='Number' AND (timeframe='",year_current,"' OR timeframe='",year_minus1,"');")
...
countydata7 <- dbGetQuery(con,countydata7_sql) %>%
  ...
  mutate(date_description='Data is a snapshot from February.') %>%
    mutate(data_source='North Dakota Department of Human Services') %>%
  mutate(data_notes='Capacity data only licensed facilities or licensed school age programs.') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_current) ~ 1,
    timeframe==all_of(year_minus1) ~0)) %>%
  mutate(dataformat='Number')
...
countydata8 <- dbGetQuery(con,countydata8_sql) %>%
  ...
  mutate(date_description='Data is a snapshot from February.') %>%
    mutate(data_source='North Dakota Department of Human Services') %>%
  mutate(data_notes='Providers include licensed facilities, licensed school age programs, and self-declaration providers.') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_current) ~ 1,
    timeframe==all_of(year_minus1) ~0)) %>%
  mutate(dataformat='Number')
...
countydata9 <- dbGetQuery(con,countydata9_sql) %>%
  ...
  #fix the range values so we have a rounded number with % sign, if applicable, a range value, if applicable, or NA
  mutate(
  data = if_else(
    grepl("^\\s*-?\\d*\\.?\\d+\\s*$", data),
    paste0(
      round(as.numeric(data), 2) * 100,
      "%"
    ),
    data))
...
data <- c(.824,.634,.708,.690,.875,.800,.674)
countydata10_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2023/24',
         varname='Four-Year Cohort Graduation Rate by Race',
...
data <- c(.111,.365,.065,.164,.342,.249)
countydata13_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2020-2024',
         varname='Children Living in Poverty by Race',
...
#####FREE OR REDUCED-PRICE LUNCH PARTICIPATION
countydata11_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_current,"' OR timeframe='",schoolyear_minus1,"');")
...
countydata11_first <- dbGetQuery(con,countydata11_sql) %>%
  ...
  mutate(data_truncated=as.character(sprintf("%.0f",round(data_truncated,1)))) %>%
