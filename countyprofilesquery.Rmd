...
data <- c(.785,.485,.817,.755,.824,.740)
...
countydata9_sql <- paste0("SELECT location, timeframe, data FROM montana WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_minus2,"' OR timeframe='",schoolyear_minus3,"');")
...
countydata9 <- dbGetQuery(con,countydata9_sql) %>%
  ...
  mutate(data_notes='') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(schoolyear_minus2) ~ 1,
...
data <- c(.861,.692,.907,.760,.785,.835,.722,.890)
...
#####WOMEN WHO RECEIVE EARLY PRENATAL CARE
race <- c("Overall","American Indian/Alaska Native","Asian","Black","White","2+ Races or Other")
data <- c(.769,.388,.685,.568,.833,.673)
countydata5_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2017-2021',
         varname='Women Who Receive Early Prenatal Care by Race',
...
#####CHILD CARE CAPACITY
countydata7_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childcarecapacitybytype' AND dataformat='Number' AND (timeframe='",year_current,"' OR timeframe='",year_minus2,"');")
...
countydata7 <- dbGetQuery(con,countydata7_sql) %>%
  ...
  mutate(data_notes='Data for 2022 includes only licensed facilities. Data for 2020 also includes facilities that are registered or otherwise approved by the state (e.g. approved relatives, registered tribal programs, and those completing a certification for in-home care).') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_current) ~ 1,
    timeframe==all_of(year_minus2) ~0)) %>%
  mutate(dataformat='Number')
...
#####PERCENT OF CHILD CARE CAPACITY PARTICIPATING IN QUALITY RATINGS
countydata8_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='childcarecapacitybyquality' AND dataformat='Percent' AND category='Not participating in QRIS' AND (timeframe='",year_current,"' OR timeframe='",year_minus2,"');")

countydata8 <- dbGetQuery(con,countydata8_sql) %>%
  mutate(data=as.numeric(paste(data))) %>%
  mutate(percent_participating=1-data) %>%
  select(-c(data)) %>%
  rename(data=percent_participating) %>%
  mutate(data=round(data,digits=3)) %>%
  mutate(data=as.character(paste(data))) %>%
  mutate(varname='Percent of Child Care Capacity Participating in Quality Rating') %>%
  mutate(age_group='Ages 0 to 13') %>%
  mutate(date_description='Snapshot from February') %>%
  mutate(data_source='North Dakota Department of Human Services') %>%
  mutate(data_notes='Only licensed providers (excluding those licensed for school age care) are included in the quality rating improvement system.') %>%
  mutate(date_mostrecent=case_when(
    timeframe==all_of(year_current) ~ 1,
    timeframe==all_of(year_minus2) ~0)) %>%
  mutate(dataformat='Percent')
...
#####FREE OR REDUCED-PRICE LUNCH PARTICIPATION
countydata9_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_minus1,"' OR timeframe='",schoolyear_minus2,"');")
...
data <- c(.870,.696,.809,.732,.907,.836,.743)
countydata10_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2020/21',
         varname='Four-Year Cohort Graduation Rate by Race',
...
#####3RD GRADE STUDENTS WHO SCORED PROFICIENT IN READING
countydata11_sql <- paste0("SELECT location, timeframe, data FROM northdakota WHERE (locationtype='County' OR locationtype='State') AND (varname='thirdgradestatereadingproficiency1year' OR varname='thirdgradestatereadingproficiency3year') AND dataformat='Percent' AND (timeframe='",schoolyear_minus1,"' OR timeframe='",schoolyear_minus3,"' OR timeframe='",threeyear_minus3,"' OR timeframe='",threeyear_minus4,"');")

reading_currentyear <- dbGetQuery(con,countydata11_sql) %>%
  subset(timeframe==schoolyear_minus1 | timeframe==threeyear_minus3) %>%
  pivot_wider(id_cols=c('location'),names_from=timeframe,values_from=data) %>%
...
reading_prioryear <- dbGetQuery(con,countydata11_sql) %>%
  ...
  mutate(data=case_when(
    ...
    oneyear=='NA' ~ threeyear)) %>%
  mutate(timeframe=case_when(
    oneyear != 'NA' ~ schoolyear_minus3,
     oneyear=='NA' ~ threeyear_minus4)) %>%
  select(c(location,timeframe,data)) %>%
  mutate(date_mostrecent=0)
...
data <- c(.111,.385,.065,.155,.345,.221)
countydata13_byrace <- data.frame(race,data) %>%
  mutate(location='North Dakota',
         timeframe='2017-2021',
         varname='Children Living in Poverty by Race',
...
#####FREE OR REDUCED-PRICE LUNCH PARTICIPATION
countydata11_sql <- paste0("SELECT location, timeframe, data FROM southdakota WHERE (locationtype='County' OR locationtype='State') AND varname='freereducedlunch' AND dataformat='Percent' AND (timeframe='",schoolyear_minus2,"' OR timeframe='",schoolyear_minus3,"');")
