...
## ############################ ##
```{r}
act_composite <- read.csv("/Users/xannaburg/Desktop/ND Education/ACT_Composite_HSgrads.csv")

############
#COUNTY DATA
act_composite_county <- act_composite %>%
  select(c(County,X2019.Composite.Avg)) %>%
  full_join(countyids,by=c('County'='ndcounty_number')) %>%
  select(-c(County)) %>%
  rename(location=ndcounty_name,
         data=X2019.Composite.Avg) %>%
...
act_allstudents <- read.csv("/Users/xannaburg/Desktop/ND Education/ACT_AllSubjects.csv") %>%
