...
```{r}
#import data
homeless_data <- read_excel(paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Homeless_",year,".xlsx")) %>%
  #rename columns removing year
  rename(denominator=all_of(paste0("20",year," Denominator")),
         numerator=all_of(paste0("20",year," Homeless")))
...
```{r}
#import data
grad_data <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Grad_Rates_",year,".xlsx"),guess_max = 1048576) %>%
...
STEP 1: Import and Clean Data
```{r}
grad_data2 <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Grad_Rates_",year,".xlsx"),guess_max = 1048576) %>%
  rename(ontime_Count='On-Time Grads',
         Total_Students='Cohort') %>%
...
#import data
dropout_data <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Dropout_Rates_7th-12th_",year,".xlsx"),guess_max = 1048576) 


############
#COUNTY DATA
dropout_county <- dropout_data %>%
  subset(Entity_Level=='Distict') %>%
  separate(col=Entity_ID,into=c('county','district'),sep=2) %>%
  mutate(county=as.numeric(paste(county))) %>%
  ...
  mutate(UnDuplicated_Dropout_Count=replace(UnDuplicated_Dropout_Count,is.na(UnDuplicated_Dropout_Count),0)) %>%
  mutate(UnDuplicated_Total_Students=replace(UnDuplicated_Total_Students,is.na(UnDuplicated_Total_Students),0)) %>%
  ...
  summarise(dropout_number=sum(UnDuplicated_Dropout_Count),
            totalenroll=sum(UnDuplicated_Total_Students),
            dropout_percent=sum(UnDuplicated_Dropout_Count)/sum(UnDuplicated_Total_Students)) %>%
  ungroup %>%
...
```{r}
#import data
assessment_data <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Assessment_Grade3_",year,".xlsx"),guess_max = 1048576) %>%
...
assessment_county <- assessment_data %>%
  ...
  mutate(proficientstudents=Nbr_Students_Proficient+Nbr_Students_Advanced) %>%
  mutate(denominator=Nbr_Students_Novice+Nbr_Students_Partially_Proficient+Nbr_Students_Proficient+Nbr_Students_Advanced) %>%
...
#import data
assessment_data1 <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Assessment_Grade3_",yearfile1,".xlsx"),guess_max = 1048576) %>%
...
assessment_data3 <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Assessment_Grade3_",yearfile3,".xlsx"),guess_max = 1048576) %>%
