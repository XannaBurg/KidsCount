...
#import data
homeless_data <- read.csv(paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Homeless_",year,".csv")) %>%
  #rename columns removing year
  rename(denominator=all_of(paste0("X20",year_periods,".Denominator")),
         numerator=all_of(paste0("X20",year_periods,".Homeless")))
...
```{r}
#import data
grad_data <- read.csv(paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Grad_Rates_",year,".csv")) %>%
...
tribalschools_updated <- tribalschools %>%
  ...
  mutate(first=replace(first,first=='03029','3029')) %>%
  mutate(first=replace(first,first=='03016','3016')) %>%
  mutate(first=replace(first,first=='03030','3030')) %>%
  mutate(first=replace(first,first=='03900','3900'))
...
#CHANGE TO THE CORRECT COUNTY NAME HERE
#**Choose the county with the next lowest count of births; if there is a tie, suppress both
#grad_all$data[grad_all$location=='Sargent'] <- 'NA'
...
#import data
dropout_data <- read.csv(paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Dropout_Rates_7th-12th_",year,".csv")) 
...
```{r}
#import data
assessment_data <- read.csv(paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Assessment_Grade3_",year,".csv")) %>%
...
assessment_county <- assessment_data %>%
  ...
  mutate(County=as.numeric(paste(County))) %>%
  left_join(countyids,by=c("County"="ndcounty_number")) %>%
  select(-c(FAY_Nbr_Students_Novice,FAY_Nbr_Students_Partially_Proficient,FAY_Nbr_Students_Proficient,FAY_Nbr_Students_Advanced)) %>%
  distinct() %>%
...
assessment_data3 <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/Assessment_Grade3_",yearfile3,".xlsx")) %>%
