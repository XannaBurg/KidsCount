...
```{r}
#import data
frl_data <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/ND Kids Count 2025 Request/1-InclusiveEnrollment.xlsx"),guess_max = 1048576)
...
frl_all <- frl_county %>%
  ...
  mutate(frl_percent_suppressed=case_when(
    ...
    suppress_denominator==0 & suppress_0to1_300==0 & suppress_99to100_300==0 & 
      ...
      suppress_0to10_21==0 & suppress_90to100_21==0  ~ frl_percent)) %>%
  ...
  mutate(frl_number_suppressed=case_when(
    ...
    suppress_denominator==0 & suppress_0to1_300==0 & suppress_99to100_300==0 & 
      suppress_0to2_101==0 & suppress_98to100_101==0 & 
      suppress_0to5_41==0 & suppress_95to100_41==0 & 
      suppress_0to10_21==0 & suppress_90to100_21==0  ~ frl_number)) %>%
...
#before suppressing the next lowest county, check whether there is also an issue within a region that could be a better fit.
#check all counties that match the region
temp_testsuppression2 <- frl_all %>%
  subset(locationtype=='County') %>%
  subset(dataformat=='Percent') %>%
  left_join(regionids,by=c('location'='county')) %>%
  mutate(suppressed_indicator=case_when(
    data=='NA' ~1,
    TRUE ~0)) %>%
  group_by(region) %>%
  summarise(sumNA=sum(suppressed_indicator)) %>%
  subset(sumNA==1)

temp_rows2 <- as.numeric(nrow(temp_testsuppression2))
if (temp_rows2==0) {
  'no additional data suppression needed'
} else if (temp_rows2>=1) {
  View(temp_testsuppression2)
}

temp_checkregions <- frl_all %>%
  subset(locationtype=='County') %>%
  left_join(regionids,by=c('location'='county')) %>%
...
#frl_all$data[frl_all$location=='Billings' & frl_all$dataformat=='Percent'] <- 'NA'


#Then check again
#4. Check that regions will not make counties identifable
temp_testsuppression3 <- frl_all %>%
  subset(locationtype=='County') %>%
  subset(dataformat=='Percent') %>%
  left_join(regionids,by=c('location'='county')) %>%
  mutate(suppressed_indicator=case_when(
...
temp_rows3 <- as.numeric(nrow(temp_testsuppression3))
if (temp_rows3==0) {
  'no additional data suppression needed'
} else if (temp_rows3>=1) {
  View(temp_testsuppression3)
}
...
```{r}
#import data
specialed_data <- read_excel(paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/ND Kids Count 2025 Request/2-SpecialEdSnapshot.xlsx"))
...
```{r}
#import data
enroll_data <- read_excel(path=paste0("/Users/xannaburg/Documents/DO NOT DELETE/ND Education/ND Kids Count 2025 Request/1-InclusiveEnrollment.xlsx"),guess_max = 1048576)
...
specialed_enroll <- specialed_all %>%
  ...
  mutate(specialed_percent_suppressed=case_when(
    ...
    suppress_denominator==0 & suppress_0to1_300==0 & suppress_99to100_300==0 & 
      suppress_0to2_101==0 & suppress_98to100_101==0 & 
      suppress_0to5_41==0 & suppress_95to100_41==0 & 
      suppress_0to10_21==0 & suppress_90to100_21==0 ~ specialed_percent)) %>%
  ...
  mutate(specialed_number_suppressed=case_when(
    ...
    suppress_denominator==0 & suppress_0to1_300==0 & suppress_99to100_300==0 & 
      suppress_0to2_101==0 & suppress_98to100_101==0 & 
      suppress_0to5_41==0 & suppress_95to100_41==0 & 
      suppress_0to10_21==0 & suppress_90to100_21==0  ~ specialed_number)) %>%
  
  #suppress Rolette and Turtle Mountain
  mutate(specialed_number_suppressed=replace(specialed_number_suppressed,location=='Rolette','NA')) %>%
  mutate(specialed_percent_suppressed=replace(specialed_percent_suppressed,location=='Rolette','NA')) %>%
  
  mutate(specialed_number_suppressed=replace(specialed_number_suppressed,location=='Turtle Mountain Reservation and Off-Reservation Trust Land (North Dakota portion)','NA')) %>%
  mutate(specialed_percent_suppressed=replace(specialed_percent_suppressed,location=='Turtle Mountain Reservation and Off-Reservation Trust Land (North Dakota portion)','NA')) %>%
...
#specialed_enroll$specialed_enroll[specialed_enroll$location=='Billings' & specialed_enroll$dataformat=='Number'] <- 'NA'
#specialed_enroll$data[specialed_enroll$location=='Billings' & specialed_enroll$dataformat=='Percent'] <- 'NA'
...
datacenter_sql <- paste0("SELECT locationid, location, timeframe, dataformat, data FROM northdakota WHERE timeframe='",fullyear,"' AND varname='totalspecialeducation' AND locationtype<>'Planning Region';")
